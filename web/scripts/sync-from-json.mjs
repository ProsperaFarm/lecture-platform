...
async function syncDatabase() {
  ...
  try {
    ...
    for (const module of course.modules) {
      for (const section of module.sections) {
        for (const lesson of section.lessons) {
          // Check if lesson exists and if YouTube URL changed
          const existingLesson = await client.query(
            `SELECT "lessonId", "youtubeUrl" FROM lessons WHERE "lessonId" = $1`,
            [lesson.id]
          );
    ...
    if (newYouTubeUrls > 0) {
      ...
      console.log(`\n💡 No new YouTube URLs found. Run this script after uploading videos.\n`);
    }
